...
// 假设 k 是一个全局变量，表示需要判断的次数
// 假设 messages 是一个全局数组，存储了按时间顺序收到的消息
let lastMessage = null; // 上一次收到的消息
let counter = 0; // 连续出现的次数
let count_set=5;

function checkMessage(message) {
  // 将当前消息添加到 messages 数组中
  if (message == lastMessage) {
    // 如果当前消息和上一次相同，增加计数器
    counter++;
  } else {
    // 如果当前消息和上一次不同，重置计数器，并更新上一次的消息
    counter = 1;
    lastMessage = message;
  }
  // 检查计数器是否达到了 k
  return counter == count_set;
}
...
var net_bug_cnt_max = 1;//最大上限,通过调整这个数可以翻倍等待时间
...
setInterval(function(){
  ...
  else
  {
    ...
    if(locked)
    {
      ...
      let last_nums=cur_message.slice(-4);
//      console.log("cur_message: "+cur_message);
      console.log("Ensruing...")
      if (checkMessage(cur_message))
      {
